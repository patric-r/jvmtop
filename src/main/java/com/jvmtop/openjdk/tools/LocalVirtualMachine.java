...
public class LocalVirtualMachine {
	...
	public static Map<Integer, LocalVirtualMachine> getNewVirtualMachines(Map<Integer, LocalVirtualMachine> existingVmMap) {
		Map<Integer, LocalVirtualMachine> map = new HashMap<Integer, LocalVirtualMachine>(existingVmMap);
		getMonitoredVMs(map, existingVmMap);
		getAttachableVMs(map, existingVmMap);
		return map;
	}

	private static void getMonitoredVMs(Map<Integer, LocalVirtualMachine> map, Map<Integer, LocalVirtualMachine> existingMap) {
		// Unsupported on J9
		if (J9Mode)
			return;
		MonitoredHost host;
	...
	public static LocalVirtualMachine getDelegateMachine(VirtualMachine vm) throws IOException {
		...
		address = (String) agentProps.get(LOCAL_CONNECTOR_ADDRESS_PROP);
		vm.detach();
		return new LocalVirtualMachine(Integer.parseInt(vm.id()), name, attachable, address);
	}
	...
	private void loadManagementAgent() throws IOException {
		...
		if (J9Mode) {
			...
			this.address = ((String) localProperties.get("com.sun.management.jmxremote.localConnectorAddress"));
		} else {
			Properties agentProps = vm.getAgentProperties();
			address = (String) agentProps.get(LOCAL_CONNECTOR_ADDRESS_PROP);
		}
